...
def grid_to_map_cloud(voxel_size, grid, threshold=0.0, origin=None):
    ...
    mask = grid_values >= threshold
    indices = grid_indices[mask]
    values = grid_values[mask][:, None]

    points = (indices.astype(np.float32) + 0.5) * float(voxel_size) + origin
    return points, values

def map_cloud_to_grid(voxel_size, points, values, grid_shape=(40,40,40), origin=None):

    grid = np.zeros(grid_shape, dtype=np.float32)

    indices = np.floor((points - origin) / float(voxel_size)).astype(int)

    valid = np.all((indices >= 0) & (indices < np.array(grid_shape)), axis=1)
...
def map_clouds_to_grid_instance(voxel_size, points, instance_ids, values=None,
                                grid_shape=(40, 40, 40), origin=None):
    ...
    if values is not None:
        values = values[valid_mask].reshape(-1)
    ...
    for i, uid in enumerate(unique_ids):
        ...
        if idx.size == 0:
            continue
        if values is None:
            grid[idx[:, 0], idx[:, 1], idx[:, 2], i] = 1.0
        else:
            grid[idx[:, 0], idx[:, 1], idx[:, 2], i] = values[mask]
...
def grid_to_map_clouds_instance(voxel_size, occupancy_voxel_instance, threshold=0.0,origin=None):
    ...
    for i in range(num_instances):
        ...
        pts = (idx.astype(np.float32) + 0.5) * float(voxel_size) + origin
